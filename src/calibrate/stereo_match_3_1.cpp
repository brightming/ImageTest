...
//        if (event == 2)
//        {
//            cout<<"("<<x<<","<<y<<") --> "<<xyz->at<cv::Point3f>(y, x).z<<endl;
//            ;
//        }
        if (event == 1)
        {
...
//    }
    ...
    //---hard code---//
    intrinsic_filename="stereo_intrinsics.yml";
    extrinsic_filename="stereo_extrinsics.yml";
    alg=STEREO_SGBM;
    disparity_filename="disparity_file.jpeg";
    point_cloud_filename="point_cloud_file.yml";
    scale=1;
...
//    imshow("roi_l",img1(roi1));
//    imshow("roi_r",img2(roi1));

    imwrite("left-rectify.jpeg",img1);
    imwrite("right-rectify.jpeg",img2);
    ...
    if( !no_display )
    {
        namedWindow("left", WINDOW_AUTOSIZE|WINDOW_NORMAL);
        cvSetMouseCallback("left", onMouse, &xyz);
        imshow("left", img1);
        namedWindow("right", 1);
        imshow("right", img2);
    ...
    int sy=1243;
    int sx=692;
    float dist=xyz.at<Point3f>(sy,sx).z;
    cout<<"sx="<<sx<<",sy="<<sy<<",dist="<<dist<<endl;
