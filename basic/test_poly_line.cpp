...
void rot(){
    Mat img=imread("/home/gumh/Pictures/page17-new-2.jpg");


    Mat img2;
    cvtColor(img,img2,CV_BGR2GRAY);
    equalizeHist(img2,img2);
    imwrite("/home/gumh/Pictures/equal-17.jpg",img2);
    return;

    RNG rng=theRNG();
    float pi=3.1415926;
    Mat draw=img.clone();
    int start_y=img.rows/2;
    int cent_x=img.cols/2;
    float theta=2;
//    for(float theta=0;theta<20;theta+=2){
        ...
        RotatedRect rRect = RotatedRect(Point2f(cent_x,start_y), Size2f(img.cols,img.rows), -theta);
        Point2f vertices[4];
        ...
        imshow("r1",cropped);
        imwrite("/home/gumh/Pictures/page17-new.jpg",cropped);
        waitKey(0);
//    }
}
